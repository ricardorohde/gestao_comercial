object FrFiltro_Fornecedor_Ctas_Pagar: TFrFiltro_Fornecedor_Ctas_Pagar
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'FrFiltro_Fornecedor_Ctas_Pagar'
  ClientHeight = 160
  ClientWidth = 389
  Color = clGray
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = Pop00
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    AlignWithMargins = True
    Left = 2
    Top = 38
    Width = 385
    Height = 82
    Margins.Left = 2
    Margins.Top = 2
    Margins.Right = 2
    Margins.Bottom = 0
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    ParentBackground = False
    TabOrder = 0
    object Label2: TLabel
      Left = 6
      Top = 39
      Width = 55
      Height = 13
      Caption = 'Fornecedor'
    end
    object Label3: TLabel
      Left = 4
      Top = 1
      Width = 45
      Height = 13
      Caption = 'Selecione'
    end
    object Label1: TLabel
      Left = 194
      Top = 1
      Width = 36
      Height = 13
      Caption = 'Per'#237'odo'
    end
    object Label4: TLabel
      Left = 285
      Top = 20
      Width = 4
      Height = 13
      Caption = '-'
    end
    object eFornecedor: TRzButtonEdit
      AlignWithMargins = True
      Left = 6
      Top = 54
      Width = 374
      Height = 21
      Margins.Left = 6
      Margins.Top = 2
      Margins.Right = 6
      Margins.Bottom = 2
      Text = ''
      DisabledColor = clWindow
      Enabled = False
      TabOrder = 3
      TextHint = 'F2 - Selecione'
      TextHintVisibleOnFocus = True
      AllowKeyEdit = False
      ButtonShortCut = 113
      AltBtnWidth = 15
      ButtonWidth = 18
      OnButtonClick = SelecionarCliente1Click
    end
    object eAgrupado: TJvComboBox
      Left = 4
      Top = 16
      Width = 184
      Height = 21
      Style = csDropDownList
      TabOrder = 0
      Text = 'Por Data de Vencto'
      OnChange = eAgrupadoChange
      Items.Strings = (
        'Por Data de Vencto'
        'Por Fornecedor'
        'Filtrar Por Fornecedor')
      ItemIndex = 0
    end
    object eDataIni: TJvDateEdit
      Left = 194
      Top = 16
      Width = 85
      Height = 21
      ShowNullDate = False
      TabOrder = 1
      OnExit = eDataIniExit
    end
    object eDataFim: TJvDateEdit
      Left = 295
      Top = 16
      Width = 85
      Height = 21
      ShowNullDate = False
      TabOrder = 2
      OnExit = eDataIniExit
    end
  end
  object RzDialogButtons1: TRzDialogButtons
    AlignWithMargins = True
    Left = 2
    Top = 122
    Width = 385
    Margins.Left = 2
    Margins.Top = 2
    Margins.Right = 2
    Margins.Bottom = 2
    CaptionCancel = 'Cancelar'
    HotTrack = True
    OKDefault = False
    ModalResultOk = 0
    OnClickOk = RzDialogButtons1ClickOk
    Color = clMaroon
    GradientColorStyle = gcsCustom
    GradientColorStart = 164
    GradientColorStop = clMaroon
    TabOrder = 1
    VisualStyle = vsGradient
  end
  object RzPanel2: TRzPanel
    AlignWithMargins = True
    Left = 2
    Top = 2
    Width = 385
    Height = 34
    Margins.Left = 2
    Margins.Top = 2
    Margins.Right = 2
    Margins.Bottom = 0
    Align = alTop
    BorderOuter = fsNone
    BorderHighlight = clNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -21
    Font.Name = 'Calibri'
    Font.Style = []
    GradientColorStart = clSilver
    GradientColorStop = 3815994
    ParentFont = False
    TabOrder = 2
    VisualStyle = vsGradient
    object Label9: TLabel
      AlignWithMargins = True
      Left = 41
      Top = 1
      Width = 147
      Height = 32
      Margins.Left = 5
      Margins.Top = 1
      Margins.Right = 1
      Margins.Bottom = 1
      Align = alLeft
      Caption = 'Contas a Pagar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -21
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Layout = tlCenter
      ExplicitHeight = 24
    end
    object Image2: TImage
      AlignWithMargins = True
      Left = 3
      Top = 1
      Width = 32
      Height = 32
      Margins.Top = 1
      Margins.Right = 1
      Margins.Bottom = 1
      Align = alLeft
      AutoSize = True
      Picture.Data = {
        }
      ExplicitLeft = 6
    end
  end
  object tbDados: TFDQuery
    Connection = FrModuloRet.DBConexao
    SQL.Strings = (
      'SELECT'
      
        '   CASE T1.PAG_STATUS -- EXIBE A QUANTIDADE DE DIAS A VENCER OU ' +
        'VENCIDO'
      '      WHEN 90 THEN IIF(CURRENT_DATE > T1.PAG_VENCIMENTO'
      
        '                      ,DATEDIFF(DAY, T1.PAG_VENCIMENTO, CURRENT_' +
        'DATE)'
      '                      ,0'
      '                   )'
      '      WHEN 91 THEN 0'
      '      WHEN 92 THEN 0'
      '   END AS PAG_DIAS_VENCIDOS,'
      ''
      
        '   CASE T1.PAG_STATUS -- EXIBE O STATUS EM QUE SE ENCONTRA A PAR' +
        'CELA'
      '      WHEN 90 THEN IIF(CURRENT_DATE > T1.PAG_VENCIMENTO'
      '                      ,'#39'Em Atraso'#39
      '                      ,'#39'A Vencer'#39
      '                   )'
      '      WHEN 91 THEN '#39'Quitada'#39
      '   END AS PAG_STATUS_TEXTO,'
      '   T2.ID AS ID_C10,'
      '   T1.ID AS ID_C11,'
      '   T1.ID_C000210,'
      '   T1.PAG_PARCELA,'
      '   T1.PAG_VENCIMENTO,'
      '   CAST(T2.PAG_DATA_INCLUSAO AS DATE) AS PAG_DATA_INCLUSAO,'
      '   T1.PAG_DOCUMENTO,'
      '   T1.PAG_VALOR,'
      '   T1.PAG_VLR_ATUALIZADO,'
      '   T1.PAG_VLR_RECEBIDO,'
      '   T1.PAG_VLR_SALDO,'
      '   T1.PAG_ANOTACOES,'
      
        '   CASE T2.PAG_STATUS -- EXIBE A QUANTIDADE DE DIAS A VENCER OU ' +
        'VENCIDO'
      '      WHEN 90 THEN IIF(CURRENT_DATE > T1.PAG_VENCIMENTO'
      '                      ,92'
      '                      ,90'
      '                   )'
      '      WHEN 91 THEN 91'
      '   END AS PAG_STATUS,'
      '   T2.PAG_ORIGEM,'
      '   T2.ID_FORNECEDOR,'
      '   T2.PAG_FORNECEDOR'
      ''
      'FROM'
      '   C000211 AS T1'
      'INNER JOIN'
      '   C000210 AS T2'
      'ON'
      '   T2.ID = T1.ID_C000210'
      'WHERE'
      '   T2.ID_EMPRESA = :ID_EMPRESA'
      'AND '
      '   T1.PAG_VENCIMENTO BETWEEN :A_DATA AND :B_DATA '
      '')
    Left = 6
    Top = 128
    ParamData = <
      item
        Name = 'ID_EMPRESA'
        DataType = ftWideString
        ParamType = ptInput
        Value = '1'
      end
      item
        Name = 'A_DATA'
        DataType = ftDate
        ParamType = ptInput
        Value = 43101d
      end
      item
        Name = 'B_DATA'
        DataType = ftDate
        ParamType = ptInput
        Value = 43831d
      end>
    object tbDadosID_C10: TLargeintField
      FieldName = 'ID_C10'
      Origin = 'ID_C10'
      Required = True
    end
    object tbDadosID_C11: TLargeintField
      FieldName = 'ID_C11'
      Origin = 'ID_C11'
      Required = True
    end
    object tbDadosID_C000210: TLargeintField
      FieldName = 'ID_C000210'
      Origin = 'ID_C000210'
    end
    object tbDadosPAG_PARCELA: TIntegerField
      Alignment = taCenter
      FieldName = 'PAG_PARCELA'
      Origin = 'PAG_PARCELA'
      DisplayFormat = '00'
    end
    object tbDadosPAG_VENCIMENTO: TDateField
      FieldName = 'PAG_VENCIMENTO'
      Origin = 'PAG_VENCIMENTO'
    end
    object tbDadosPAG_DIAS_VENCIDOS: TLargeintField
      Alignment = taCenter
      FieldName = 'PAG_DIAS_VENCIDOS'
      Origin = 'PAG_DIAS_VENCIDOS'
      DisplayFormat = '000'
    end
    object tbDadosPAG_DATA_INCLUSAO: TDateField
      FieldName = 'PAG_DATA_INCLUSAO'
      Origin = 'PAG_DATA_INCLUSAO'
    end
    object tbDadosPAG_DOCUMENTO: TStringField
      FieldName = 'PAG_DOCUMENTO'
      Origin = 'PAG_DOCUMENTO'
      Size = 25
    end
    object tbDadosPAG_ANOTACOES: TStringField
      FieldName = 'PAG_ANOTACOES'
      Origin = 'PAG_ANOTACOES'
      Size = 120
    end
    object tbDadosPAG_STATUS: TIntegerField
      FieldName = 'PAG_STATUS'
      Origin = 'PAG_STATUS'
    end
    object tbDadosPAG_STATUS_TEXTO: TStringField
      FieldName = 'PAG_STATUS_TEXTO'
      Origin = 'PAG_STATUS_TEXTO'
      FixedChar = True
      Size = 9
    end
    object tbDadosPAG_ORIGEM: TIntegerField
      FieldName = 'PAG_ORIGEM'
      Origin = 'PAG_ORIGEM'
    end
    object tbDadosPAG_FORNECEDOR: TStringField
      FieldName = 'PAG_FORNECEDOR'
      Origin = 'PAG_FORNECEDOR'
      Size = 120
    end
    object tbDadosID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Origin = 'ID_FORNECEDOR'
    end
    object tbDadosPAG_VALOR: TFMTBCDField
      FieldName = 'PAG_VALOR'
      Origin = 'PAG_VALOR'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 2
    end
    object tbDadosPAG_VLR_ATUALIZADO: TFMTBCDField
      FieldName = 'PAG_VLR_ATUALIZADO'
      Origin = 'PAG_VLR_ATUALIZADO'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 2
    end
    object tbDadosPAG_VLR_RECEBIDO: TFMTBCDField
      FieldName = 'PAG_VLR_RECEBIDO'
      Origin = 'PAG_VLR_RECEBIDO'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 2
    end
    object tbDadosPAG_VLR_SALDO: TFMTBCDField
      FieldName = 'PAG_VLR_SALDO'
      Origin = 'PAG_VLR_SALDO'
      DisplayFormat = ',0.00'
      Precision = 18
      Size = 2
    end
  end
  object Pop00: TPopupMenu
    AutoPopup = False
    Left = 38
    Top = 128
    object SelecionarCliente1: TMenuItem
      Caption = 'Selecionar Cliente'
      ShortCut = 113
      OnClick = SelecionarCliente1Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object Sair1: TMenuItem
      Caption = 'Sair'
      ShortCut = 27
      OnClick = Sair1Click
    end
  end
end
