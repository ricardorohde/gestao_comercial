...
procedure TFrMarcasProdutos.dsDadosStateChange(Sender: TObject);
begin
...
procedure TFrMarcasProdutos.tbDadosAfterPost(DataSet: TDataSet);
begin
   DataSet.Refresh;
end;

procedure TFrMarcasProdutos.tbDadosBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFrMarcasProdutos.tbDadosBeforePost(DataSet: TDataSet);
begin
...
procedure TFrMarcasProdutos.tbDadosMRC_STATUSGetText(Sender: TField; var Text: string; DisplayText: Boolean);
begin
   if DisplayText then
   begin
...
procedure TFrMarcasProdutos.tbDadosNewRecord(DataSet: TDataSet);
begin
   {Dados básicos do cadastros}
   DataSet.FieldByName('ID_EMPRESA').AsInteger    := FEmpresaClass.ID;
   DataSet.FieldByName('MRC_DATA_REG').AsDateTime := now;
   DataSet.FieldByName('MRC_STATUS').AsInteger    := 1;
end;
