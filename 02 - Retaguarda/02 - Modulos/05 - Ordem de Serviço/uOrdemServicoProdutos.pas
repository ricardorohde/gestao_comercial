...
type
  TFrOrdemServicoProdutos = class(TForm)
    pnProduto: TPanel;
    Label3: TLabel;
    Label4: TLabel;
    Label5: TLabel;
  ...
  public
    { Public declarations }
    SqlText : String;
    IDTabelaPrecos : Integer;
    Vr_Forma_Inc_Produto : Integer;
  end;

var
  FrOrdemServicoProdutos: TFrOrdemServicoProdutos;

implementation

uses
   uListaProdutos, Classe.Empresa, uEmissorNfe, uListaNCMs, uOrdemServico,
  uModuloRet;

{$R *.dfm}

procedure TFrOrdemServicoProdutos.ed_qtdeExit(Sender: TObject);
begin
...
procedure TFrOrdemServicoProdutos.FormCreate(Sender: TObject);
begin
...
procedure TFrOrdemServicoProdutos.FormShow(Sender: TObject);
var
   OrderBy : String;
begin

   // Ajusta conforme a UF do destinatário
   SqlText := SqlText + ' AND ' + dsNfes.DataSet.FieldByName('os_cli_uf').AsString + ' = :UF';
   ...
   tbTribs.ParamByName('trb_operacao').AsInteger := 1;
   tbTribs.ParamByName('UF').AsInteger           := 1;
   tbTribs.Open();
end;

procedure TFrOrdemServicoProdutos.ed_codigoEnter(Sender: TObject);
begin
   TEdit(Sender).Color := $00DFDFDF;
end;

procedure TFrOrdemServicoProdutos.ed_codigoExit(Sender: TObject);
begin
   TEdit(Sender).Color := clWindow;
end;

procedure TFrOrdemServicoProdutos.ed_codigoKeyPress(Sender: TObject; var Key: Char);
begin
   // Se for digitado algo neste campo, altera o tipo de inclusão para manual
   Vr_Forma_Inc_Produto := 0;
end;

procedure TFrOrdemServicoProdutos.ed_descricaoExit(Sender: TObject);
begin
   TEdit(Sender).Color := $00DDFFFF;
end;

procedure TFrOrdemServicoProdutos.ed_ncmButtonClick(Sender: TObject);
begin
   try
...
procedure TFrOrdemServicoProdutos.RzDialogButtons1ClickOk(Sender: TObject);
begin
...
procedure TFrOrdemServicoProdutos.Sair1Click(Sender: TObject);
begin
   Self.Close;
end;

procedure TFrOrdemServicoProdutos.SelecionarProduto1Click(Sender: TObject);
begin

   try

      FrListaProdutos           := TFrListaProdutos.Create(self);
      FrListaProdutos.TipoQuery := 1; // Cadastro simples do produtos
      ...
      begin
         ...
         begin
            ...
            FieldByName('prd_vlr_unitario').AsFloat   := 0.00;
            FieldByName('prd_vlr_subtotal').AsFloat   := 0.00;

            // Seleciona o código interno ou o código de terceiros
      ...
      // Se for digitado algo neste campo, altera o tipo de inclusão para automática
